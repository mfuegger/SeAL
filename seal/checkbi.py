...
def findBoundary(
    ...
    # check if all marked (only initially)
    elif initially and isSusceptible(
        t=tfrom,
        s=s,
        states=states,
        s_state=s_state,
            ...
            if middle_is_M:
                if not start_is_M_initially:
                    ...
                    return findBoundarySA(
                        ...
                        output_signals=output_signals,
                        SAF=SAF,
                        start_is_M_initially=start_is_M_initially,
                        snk_delay=snk_delay,
                        src_delay=src_delay,
                        initially=False,
                        monitor=monitor,
    ...
    if victim_signals:
        ...
        for s in victims:
            ...
            for i in range(len(times) - 1):
                tfrom = times[i]
                tto = times[i + 1]
                start_is_M_initially = isSusceptibleSA(
                    t=tfrom,
                    s=s,
                    states=states,
                    events=events,
                ...
                assert (
                    boundary - tfrom >= 0
                ), f"signal = {s}, tfrom = {tfrom}, boundary = {boundary}, tto = {tto}"
